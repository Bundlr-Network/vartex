...
export const findTxIDsFromTxFilters = async (
  ...
  const isBucketSearchTag = Boolean(
    R.isEmpty(txFilterKeys.filter(i => i !== "ids")) && !R.isEmpty(queryParameters.tags)
  );
  ...
  if (isBucketSearchTx) {
    ...
    const idsFilter = queryParameters.ids?.length > 0 ? `AND tx_id IN ('${queryParameters.ids.join("','").slice(0, -2)})` : "";
